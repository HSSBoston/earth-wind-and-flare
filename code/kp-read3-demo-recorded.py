...
def cMajorWithLongTonesRests(kpValues):
    ...
    for (kpValue, group) in adjustedKpList:
        try:
            if kpValue == "0 ":
                ...
                sound.playTone(stream, 0, eNote * len(group), 0)
            else:
                sound.playTone(stream, cMajorScale[ kp.index(kpValue) ], eNote * len(group), 0.5)
        except KeyboardInterrupt:
            break

def cMajorWithLongTonesRestsIntensity(kpValues):
    adjustedKpList = [(kpValue, list(group)) for kpValue, group in groupby(kpValues)]
    # e.g., [('1-', ['1-']), ('0+', ['0+', '0+']), ...]
    fiveOrHigherKp = ["5-", "5 ", "5+", "6-", "6 ", "6+", "7-", "7 ", "7+", "8-", "8 ", "8+", "9-", "9"]
    allSamples = np.empty(0, dtype=np.float32)
    for (kpValue, group) in adjustedKpList:
        try:
            if kpValue == "0 ":
                #time.sleep(eNote * len(group))
                samples = sound.playTone(stream, 0, eNote * len(group), 0)
                allSamples = sound.concatnateSamples(allSamples, samples)
            else:
                if kpValue in fiveOrHigherKp:
                    print("Intensity increased: ", kpValue)
                    samples = sound.playTone(stream, cMajorScale[ kp.index(kpValue) ], eNote * len(group))
                    allSamples = sound.concatnateSamples(allSamples, samples)
                else:
                    samples = sound.playTone(stream, cMajorScale[ kp.index(kpValue) ], eNote * len(group), 0.5)
                    allSamples = sound.concatnateSamples(allSamples, samples)
        except KeyboardInterrupt:
            break
    sound.saveSamplesAsWav(allSamples, "cmajor.wav")

def cBluesWithLongTonesRestsIntensity(kpValues):
    adjustedKpList = [(kpValue, list(group)) for kpValue, group in groupby(kpValues)]
    # e.g., [('1-', ['1-']), ('0+', ['0+', '0+']), ...]
    fiveOrHigherKp = ["5-", "5 ", "5+", "6-", "6 ", "6+", "7-", "7 ", "7+", "8-", "8 ", "8+", "9-", "9"]
    allSamples = np.empty(0, dtype=np.float32)
    for (kpValue, group) in adjustedKpList:
        try:
            if kpValue == "0 ":
                ...
                if kpValue in fiveOrHigherKp:
                    print("Intensity increased: ", kpValue)
                    samples = sound.playTone(stream, cBluesScale[ kp.index(kpValue) ], eNote * len(group))
                    allSamples = sound.concatnateSamples(allSamples, samples)
                else:
                    samples = sound.playTone(stream, cBluesScale[ kp.index(kpValue) ], eNote * len(group), 0.5)
                    allSamples = sound.concatnateSamples(allSamples, samples)
        except KeyboardInterrupt:
            break
    sound.saveSamplesAsWav(allSamples, "cblues.wav")

def cMajorChordWithLongTonesRestsIntensity(kpValues):
    adjustedKpList = [(kpValue, list(group)) for kpValue, group in groupby(kpValues)]
    # e.g., [('1-', ['1-']), ('0+', ['0+', '0+']), ...]
    fiveOrHigherKp = ["5-", "5 ", "5+", "6-", "6 ", "6+", "7-", "7 ", "7+", "8-", "8 ", "8+", "9-", "9"]
    allSamples = np.empty(0, dtype=np.float32)
    for (kpValue, group) in adjustedKpList:
        try:
            if kpValue == "0 ":
                ...
                if kpValue in fiveOrHigherKp:
                    print("Intensity increased: ", kpValue)
                    samples = sound.playTones(stream, cMajorChordScale[ kp.index(kpValue) ], eNote * len(group))
                    allSamples = sound.concatnateSamples(allSamples, samples)
                else:
                    samples = sound.playTones(stream, cMajorChordScale[ kp.index(kpValue) ], eNote * len(group), 0.5)
                    allSamples = sound.concatnateSamples(allSamples, samples)
        except KeyboardInterrupt:
            break
    sound.saveSamplesAsWav(allSamples, "cmajor-chord.wav")

def cMinorHarmonicChordWithLongTonesRestsIntensity(kpValues):
    adjustedKpList = [(kpValue, list(group)) for kpValue, group in groupby(kpValues)]
    # e.g., [('1-', ['1-']), ('0+', ['0+', '0+']), ...]
    fiveOrHigherKp = ["5-", "5 ", "5+", "6-", "6 ", "6+", "7-", "7 ", "7+", "8-", "8 ", "8+", "9-", "9"]
    allSamples = np.empty(0, dtype=np.float32)
    for (kpValue, group) in adjustedKpList:
        try:
            if kpValue == "0 ":
                ...
                if kpValue in fiveOrHigherKp:
                    print("Intensity increased: ", kpValue)
                    samples = sound.playTones(stream, cMinorHarmonicChordScale[ kp.index(kpValue) ], eNote * len(group))
                    allSamples = sound.concatnateSamples(allSamples, samples)
                else:
                    samples = sound.playTones(stream, cMinorHarmonicChordScale[ kp.index(kpValue) ], eNote * len(group), 0.5)
                    allSamples = sound.concatnateSamples(allSamples, samples)
        except KeyboardInterrupt:
            break
    sound.saveSamplesAsWav(allSamples, "cminor-harmonic-chord.wav")
    
cMajorWithLongTonesRestsIntensity(kpValues[:40])
time.sleep(2)
cBluesWithLongTonesRestsIntensity(kpValues[:40])
time.sleep(2)
cMajorChordWithLongTonesRestsIntensity(kpValues[:40])
time.sleep(2)
cMinorHarmonicChordWithLongTonesRestsIntensity(kpValues[:40])
